// =====================================================
// EKAIDAN — site_js_final.js  (v5)
// Single source of truth for AUTH + DB
// Include on: index.html, music_library.html,
//             karaoke.html, auth_page.html, ekaidan-admin.html
// =====================================================
...
const AUTH = {
  ...
  async signUp(email, password, options = {}) {
    ...
    const data = await r.json().catch(() => ({ error: { message: 'Unexpected response.' } }));
  ...
  async resendConfirmation(email, redirectTo) {
    ...
    const data = await r.json().catch(() => ({ error: { message: 'Unexpected response.' } }));
    if (!r.ok) return { error: { message: data.error?.message || 'Resend failed.' } };
    return data;
  },
...
const DB = {
  ...
  getSongById(id)        { return this._req(`songs?id=eq.${id}`).then(r => r[0] || null); },
